...
/**
  ...
  **/
  ...
  function getTotalparticipants(){
    $conn = connexionBase();
    $sql_query = "SELECT DISTINCT refsportif FROM inscription JOIN evenementsportif ON inscription.refevenement = evenementsportif.idevenement WHERE YEAR(dateev)>= YEAR(CURDATE())";
    $result = pg_query($conn, $sql_query) or die("Erreur SQL count event");
