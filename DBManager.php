...
/**
  ...
  **/
  ...
  function getTotalparticipants(){
    $conn = connexionBase();
    $sql_query = "SELECT DISTINCT refsportif FROM inscription JOIN evenement sportif ON inscription.refevenement = evenementsportif.idevenement WHERE dateev>DATEADD(year,DTAEDIFF(year,0,getdate())-1.0) AND dateev<=DATEADD(year,DTAEDIFF(year,0,getdate())+1.0)";
    $result = pg_query($conn, $sql_query) or die("Erreur SQL count event");
